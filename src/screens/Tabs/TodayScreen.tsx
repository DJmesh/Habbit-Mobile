  ...
  return (
    <Screen>
          ...
          renderItem={({ item }) => {
            ...
            return (
              <View style={{ marginBottom: 12 }}>
                ...
                <View style={{ height: 8 }} />
                <Button title="Marcar agora" onPress={() => setDoneToday(item.id)} />
              </View>
            );
          }}
        />
      ...
      <Card style={{ marginBottom: 12 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          ...
          <View style={{ width: 120 }}>
            <Button title="Adicionar" onPress={() => tasks.addTask(taskTitle)} />
          </View>
        </View>
        <View style={{ height: 12 }} />
        {todayTasks.map(t => (
          <View key={t.id} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8, gap: 8 }}>
            <Ionicons name={t.done ? 'checkmark-circle' : 'ellipse-outline'} size={22} color={t.done ? colors.success : colors.subtext} onPress={() => tasks.toggleTask(t.id)} />
            <Text onPress={() => tasks.toggleTask(t.id)} style={{ color: t.done ? colors.subtext : colors.text, textDecorationLine: t.done ? 'line-through' : 'none' }}>{t.title}</Text>
          </View>
        ))}
      </Card>
      ...
      <Card>
        ...
        <Text style={{ color: colors.subtext, marginBottom: 6 }}>Total de hoje: Cal {totals.calories} • P {totals.protein}g • C {totals.carbs}g • G {totals.fat}g</Text>
        <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
          <Input placeholder="Refeição" value={mealName} onChangeText={setMealName} style={{ flex: 1 }} />
        ...
        <View style={{ height: 8 }} />
        <Button title="Lançar refeição" onPress={() => nutri.addMeal(mealName, Number(cal)||0, Number(prot)||0, Number(carb)||0, Number(fat)||0)} />
      </Card>
    </Screen>
  );
}
